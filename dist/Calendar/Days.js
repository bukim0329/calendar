...
const Days = () => {
  ...
  return /*#__PURE__*/React.createElement(DateBox, null, weeks.map((week, weekIdx) => /*#__PURE__*/React.createElement(Week, {
    key: weekIdx,
    weekInd: weekIdx
  }, week.map((day, idx) => {
    const currentDate = {
    ...
    return /*#__PURE__*/React.createElement(DateBackground, {
      key: idx,
    ...
    }, /*#__PURE__*/React.createElement(DateContent, {
      "data-year": date.year,
...
const isDuringPeriod = (periodStart, periodEnd, today) => {
  ...
  return new Date(periodStart.year, periodStart.month - 1, periodStart.date) <= new Date(today.year, today.month - 1, today.date) && new Date(today.year, today.month - 1, today.date) <= new Date(periodEnd.year, periodEnd.month - 1, periodEnd.date);
};
...
const DateContent = styled.div`
  ...
  periodStyle,
  isDuringPeriod
}) => isDuringPeriod ? periodStyle.backgroundColor : "white"};
    border-radius: 50%;
    ${({
  isPeriodStart,
  isPeriodEnd,
  clickedDateStyle
}) => isPeriodStart || isPeriodEnd ? clickedDateStyle : ""};
    &:hover {
        border-radius: 50%;
        ${({
  hoverDateStyle
